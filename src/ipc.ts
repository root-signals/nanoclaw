...
export async function processTaskIpc(
  data: {
    ...
    key?: string;
    value?: string;
    // For register_group / unregister_group from admin containers
    adminMode?: boolean;
  },
  ...
  switch (data.type) {
    ...
    case 'unregister_group':
      ...
      if (data.jid) {
        ...
        if (existing.isMain) {
          logger.warn(
            { jid: data.jid },
            'unregister_group: cannot unregister main group',
          );
          break;
        }
        deleteRegisteredGroup(data.jid);
        logger.info(
          { jid: data.jid, folder: existing.folder },
          'Group unregistered',
        );
      } else {
    ...
    case 'self_rebuild':
      if (true) {
        ...
        logger.info(
          { group: sourceGroup },
          'Admin requested rebuild and restart',
        );
        setTimeout(async () => {
          try {
        ...
        logger.warn(
          { sourceGroup },
          'Unauthorized self_rebuild attempt blocked',
        );
      }
      break;
      ...
      if (true) {
        ...
        setTimeout(async () => {
          try {
            if (data.message) {
              execSync(
                `git commit --amend -m ${JSON.stringify(data.message)}`,
                { cwd: process.cwd(), stdio: 'pipe' },
              );
            }
            const result = execSync('git push origin main 2>&1', {
              cwd: process.cwd(),
              stdio: 'pipe',
              timeout: 60000,
            });
            logger.info(
              { output: result.toString().slice(0, 500) },
              'Git push completed',
            );
          } catch (err) {
    ...
    case 'manage_env': {
      ...
      if (data.operation === 'list') {
        try {
          ...
          logger.info(
            { keys: keys.map((k: string) => k.split('=')[0]) },
            'Env list requested',
          );
        } catch (err) {
